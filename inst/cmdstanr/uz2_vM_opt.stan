...
profile("old_lik"){
  if (lumped == 0){
    ...
    //if ((old_dates[i] - tau[i]) > -1*pi()) {
      P[i] = (1 - von_mises_cdf(old_dates[i] | mu[i], kappa[i])) + (von_mises_cdf(rollunder(old_dates[i] - tau[i])| mu[i],kappa[i]));//
    //} else {
...
profile("new_lik"){
  if (lumped == 0){
    ...
    for (i in 1:N_new) {
      //if ((new_dates[i] - tau[i+N_old+N_moult]) > -1*pi()) {
        R[i] = (1 - von_mises_cdf(new_dates[i] | mu[i+N_old+N_moult], kappa[i+N_old+N_moult])) + von_mises_cdf(rollunder(new_dates[i] - tau[i+N_old+N_moult])| mu[i+N_old+N_moult],kappa[i+N_old+N_moult]);//this needs tweaking to ensure the difference is always in [-pi,pi]
      //} else {
